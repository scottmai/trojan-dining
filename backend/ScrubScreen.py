...
def ScrubScreen(Date):
    ...
    for block in MealTimes_HTML: #loop through all the meal blocks
        MealTimeName = block.find("span", class_ = "fw-accordion-title-inner").text.split()[0] #find the name of the block
        DiningHalls_HTML = block.find_all("div", class_="col-sm-6 col-md-4") #make a new list of all the dining halls in a block
        ListOfDiningHalls = []
        for Hall in DiningHalls_HTML:
            DiningHallName = Hall.find("h3",class_="menu-venue-title").text

            Station_Names_HTML = Hall.find_all("h4") 
            MenuItems_ListList = Hall.find_all("ul") #the list of foods is a bunch of lists not under stations but under DiningHalls so have to get a list of foods in each stations here
            ListOfStations = []

            for i in range(len(Station_Names_HTML)): #loop through all the stations
                StationName = Station_Names_HTML[i].text#station name
                ListOfMenuItems = [] 
                if(StationName != "No items to display for this date"): #when theres no items program bugs out; my workaround
                    MenuItems_HTML = MenuItems_ListList[i].find_all("li")#find all the food items
                    for menuItem in MenuItems_HTML:#iterate through all the food
                        MenuItemName = menuItem.find(text=True, recursive=False)#get the name of the food item
                        Allergens_HTML = menuItem.find_all("i")#get all the allergens for that food item
                        ListOfAllergens = []
    ...
    # nest the lists inside eachother
                        ...
                        ListOfMenuItems.append(MenuItem(MenuItemName, ListOfAllergens))
                ListOfStations.append(Station(StationName, ListOfMenuItems))
            ListOfDiningHalls.append(DiningHall(DiningHallName, ListOfStations))
